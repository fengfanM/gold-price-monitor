...
| 等级 | 用途 | 当前源 | 强信号规则 |
| --- | --- | --- | --- |
| critical | 直接影响黄金方向和人民币报价校准 | `GC=F`、`DX-Y.NYB`、`USDCNY=X`、`DFII10` | 失败时禁止强买/强卖，只允许观察或降级 |
...
| --- | --- | --- | --- | --- |
| Yahoo `query1/query2.finance.yahoo.com` | 可快速拿到期货、DXY、USDCNY 代理行情，但不是稳定官方授权 API | 高。端点未承诺稳定，可能 403、限流、Cookie/crumb 变化 | 适合秒级到分钟级监控，但生产需缓存和降频 | 保留为免费代理源；关键生产需接入授权行情或付费数据商 |
| FRED API/CSV | 官方宏观源，适合实际利率、通胀预期、VIX、美债 | 低。JSON API 需要 `FRED_API_KEY`，CSV fallback 更宽松但不应高频撞源 | 宏观数据日频/低频，非实时 | 配置 `FRED_API_KEY`；按小时或日缓存 |
...
| CME OI/volume | CME 官方数据权威，但当前 OI 必须配置，volume 默认用 Yahoo `GC=F` 代理 | 高。官方实时/统计数据通常有许可；Yahoo volume 不是 CME 官方源 | OI/volume 日频，Yahoo 可能缺失或延迟 | 配置 `CME_GOLD_OI_CSV_URL` 和 `CME_GOLD_VOLUME_CSV_URL` |
...
- `CME_GOLD_OI_CSV_URL` 与 `CME_GOLD_VOLUME_CSV_URL`：CME OI/volume 授权或内部清洗 CSV。
